...
class PushTEnv(gym.Env):
    ...
    def _get_obs(self):
        if self.use_sin_cos:
            ...
            block_angle = np.sin(self.block.angle), np.cos(self.block.angle)
        else:
    ...
    def _render_frame(self, mode):
        ...
        if not self.is_cluttered:
            canvas = pygame.Surface((self.window_size, self.window_size))
            canvas.fill((255, 255, 255))
            self.screen = canvas
        else:
            canvas = pygame.Surface((self.window_size, self.window_size))
            background = pygame.image.load("/home/kathy/dev/dino_wm_rebuttal/env/pusht/tst.jpg")  # Load the image
            background = pygame.transform.scale(background, (self.window_size, self.window_size))  # Scale if needed
            canvas.blit(background, (0, 0))  # Draw the background image onto the canvas
            self.screen = canvas
    ...
    def _set_state(self, state):
        ...
        pos_block = state[2:4]
        if self.use_sin_cos:
            rot_block = np.arctan2(state[4], state[5])
        else:
            rot_block = state[4]
